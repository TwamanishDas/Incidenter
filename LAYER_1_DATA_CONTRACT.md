...
| Azure Monitor Metrics (SQL + VM) | `MetricsQueryClient.query_resource` | `azure_monitor_metrics` | SQL -> `database`, VM -> `application` | SQL CPU/sessions/workers/deadlocks + VM CPU/network/disk; correlation via diagnostics when available, otherwise synthetic correlation id | every 30s, watermark window |
...
- VM metrics are ingested with additional metric fields in payload for downstream analytics/forecast extensions.
